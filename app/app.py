...
async def add_process_time_header(request: Request, call_next):
    ...
    if log.level <= log.root.level:
        ...
        start_time = time()
        log.debug("Request received, method=%s, headers=%s, url=%s" % (
            request.method, str(request.headers.raw), str(request.url)))
    ...
    if log.level <= log.root.level:
        ...
        # Save the data to a list and use iterate_in_threadpool to initiate the iterator again
